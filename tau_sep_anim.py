...
def EFT_solve(j, Lambda, path, kind, folder_name=''):
    ...
    return a, x, sigma_l, p_l, kappa_l, Phi_l, tau_l#*a**2, Phi_l#*a**2, tau_l#*a**2
...
for j in tqdm(range(1)):
    ...
    # ax.plot(x, sigma_l - sigma_l_nsc, c='seagreen', lw=1.5, ls='dashed', label=r'$\Xi^{\mathrm{m}}_{l}$')
    # ax.plot(x, sigma_l_nsc, c='cyan', lw=1.5, ls='dashed', label=r'$\Xi^{\mathrm{s}}_{l}$')
    # ax.plot(x, sigma_l, c='seagreen', lw=1.5, ls='dashed', label=r'$\Xi_{l}$')
    # ax.plot(x, -p_l, c='r', lw=1.5, ls='dashed', label=r'$-\rho_{l}U^{2}$')
    ax.plot(x, kappa_l, c='orange', lw=1.5, ls='dashdot', label=r'$\tau_{\mathrm{k}}$')
    ax.plot(x, Phi_l, c='k', lw=1.5, ls='dashdot', label=r'$\tau_{\mathrm{g}}$')
    ax.set_xlabel(r'$x/L$', fontsize=20)
    # ax.set_ylabel(r'$a^{2}\tau\;[\mathrm{M}_\mathrm{p}H_{0}^{2}L^{-1}]$', fontsize=20)
    ax.set_ylabel(r'$\tau\;[\mathrm{M}_\mathrm{p}H_{0}^{2}L^{-1}]$', fontsize=20)
    ax.minorticks_on()
    ...
    # # plt.savefig(f'../plots/paper_plots_final/tau_k/tau_{j:02}.png', bbox_inches='tight', dpi=300)
    # plt.savefig(f'../plots/paper_plots_final/tau_k1/tau_{j:02}.png', bbox_inches='tight', dpi=300)
