...
def EFT_solve(j, Lambda, path, kind, folder_name=''):
    ...
    tau_l = (kappa_l + Phi_l)
    return a, x, sigma_l*a**2, p_l*a**2, kappa_l*a**2, Phi_l*a**2, tau_l*a**2, dc_l, -dv_l/(a**(-1/2)*100)
...
fig.suptitle(rf'$\Lambda = {Lambda_int} \,k_{{\mathrm{{f}}}}$ ({kind_txt})', fontsize=22, y=0.985)

j = 11#0
a, x, sigma_l, p_l, kappa_l, Phi_l, tau_l, dc_l, dv_l = EFT_solve(j, Lambda, path, kind)
a, x, sigma_l_nsc, _, _, _, _, _, _ = EFT_solve(j, Lambda, path, kind, folder_name=folder_name)

ax[0].set_title(rf'$a = {a}$', fontsize=20)
ax[0].plot(x, tau_l, c='b', lw=2, ls='solid', label=r'$\tau$')
ax[0].plot(x, sigma_l - sigma_l_nsc, c='seagreen', lw=1.5, ls='dashed', label=r'$\Xi^{\mathrm{m}}_{l}$')
ax[0].plot(x, sigma_l_nsc, c='midnightblue', lw=1.5, ls='dashed', label=r'$\Xi^{\mathrm{s}}_{l}$')
ax[0].plot(x, p_l, c='r', lw=1.5, ls='dashed', label=r'$\rho_{l}U^{2}$')
ax[0].plot(x, kappa_l, c='orange', lw=2, ls='solid', label=r'$\tau_{\mathrm{k}}$')
ax[0].plot(x, Phi_l, c='k', lw=2, ls='solid', label=r'$\tau_{\mathrm{g}}$')
# ax[0].plot(x, dc_l, c='k', lw=1.5, label=r'$\delta_{\ell}$')
# ax[0].plot(x, dv_l, c='b', ls='dashed', lw=1.5, label=r'$\theta_{\ell}$')
# print(np.abs(dv_l).max(), np.abs(dc_l).max())

j = 23 #14
a, x, sigma_l, p_l, kappa_l, Phi_l, tau_l, dc_l, dv_l  = EFT_solve(j, Lambda, path, kind)
a, x, sigma_l_nsc, _, _, _, _, _, _ = EFT_solve(j, Lambda, path, kind, folder_name=folder_name)

ax[1].set_title(rf'$a = {a}$', fontsize=20)
ax[1].plot(x, tau_l, c='b', lw=2, ls='solid', label=r'$\tau$')
ax[1].plot(x, sigma_l - sigma_l_nsc, c='seagreen', lw=1.5, ls='dashed', label=r'$\Xi^{\mathrm{m}}_{l}$')
ax[1].plot(x, sigma_l_nsc, c='midnightblue', lw=1.5, ls='dashed', label=r'$\Xi^{\mathrm{s}}_{l}$')
ax[1].plot(x, p_l, c='r', lw=1.5, ls='dashed', label=r'$\rho_{l}U^{2}$')
ax[1].plot(x, kappa_l, c='orange', lw=2, ls='solid', label=r'$\tau_{\mathrm{k}}$')
ax[1].plot(x, Phi_l, c='k', lw=2, ls='solid', label=r'$\tau_{\mathrm{g}}$')
# ax[1].plot(x, dc_l, c='k', lw=1.5, label=r'$\delta_{\ell}$')
# ax[1].plot(x, dv_l, c='b', ls='dashed', lw=1.5, label=r'$\theta_{\ell}$')
...
ax[2].set_title(rf'$a = {a}$', fontsize=20)
ax[2].plot(x, tau_l, c='b', lw=2, ls='solid', label=r'$\tau$')
ax[2].plot(x, sigma_l - sigma_l_nsc, c='seagreen', lw=1.5, ls='dashed', label=r'$\Xi^{\mathrm{m}}_{l}$')
ax[2].plot(x, sigma_l_nsc, c='midnightblue', lw=1.5, ls='dashed', label=r'$\Xi^{\mathrm{s}}_{l}$')
ax[2].plot(x, p_l, c='r', lw=1.5, ls='dashed', label=r'$\rho_{l}U^{2}$')
ax[2].plot(x, kappa_l, c='orange', lw=2, ls='solid', label=r'$\tau_{\mathrm{k}}$')
ax[2].plot(x, Phi_l, c='k', lw=2, ls='solid', label=r'$\tau_{\mathrm{g}}$')
# ax[2].plot(x, dc_l, c='k', lw=1.5, label=r'$\delta_{\ell}$')
# ax[2].plot(x, dv_l, c='b', ls='dashed', lw=1.5, label=r'$\theta_{\ell}$')
...
# ax[0].set_ylabel(r'$\delta_{\ell}, \theta_{\ell}$', fontsize=20)
# ax[2].set_ylabel(r'$\delta_{\ell}, \theta_{\ell}$', fontsize=20)

# ax[2].yaxis.set_label_position('right')
plt.legend(fontsize=16, ncol=3)#, labelspacing=0.85, bbox_to_anchor=(1.025, 1))
