...
def tau_calc(j, Lambda, path, kind, folder_name=''):
    ...
    Phi_l = -(rho_0 / (3 * (H0**2) * (a**0))) * (grad_phi_l2 - grad_phi2_l)

    # #here is the full stress tensor; this is the object to be fitted for the EFT paramters
    tau_l = (kappa_l + Phi_l)
    return a, x, sigma_l, p_l, kappa_l, Phi_l, tau_l
...
# ax[0].plot(x, tau_l, c='b', lw=1.5, ls='solid', label=r'$[\tau]_{\Lambda}$')
...
# ax[1].plot(x, tau_l, c='b', lw=1.5, ls='solid', label=r'$[\tau]_{\Lambda}$')
...
# ax[2].plot(x, tau_l, c='b', lw=1.5, ls='solid', label=r'$[\tau]_{\Lambda}$')
...
# ax[0].set_ylabel(r'$a^{2}[\tau]_{\Lambda}\;[\mathrm{M}_\mathrm{p}H_{0}^{2}L^{-1}]$', fontsize=20)
# ax[2].set_ylabel(r'$a^{2}[\tau]_{\Lambda}\;[\mathrm{M}_\mathrm{p}H_{0}^{2}L^{-1}]$', fontsize=20)
