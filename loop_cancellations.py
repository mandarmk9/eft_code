...
# a_list, x, ac_true, ac, ac2, ac3, ac4 = alpha_c_finder(Nfiles, Lambda, path, A, mode, kind, fm=fm, npars=npars, fde_method=fde_method, folder_name=folder_name)
...
a_list, ac_true, ac, ac2, ac3, ac4, ac_pred = np.array(read_file)
file.close()
...
ax.set_title(r'$\Lambda = {} \,k_{{\mathrm{{f}}}}$ ({})'.format(int(Lambda/(2*np.pi)), kind_txt), fontsize=18, y=1.01)
...
ax.set_ylabel(r'$\eta$', fontsize=22)
...
ratio_true = 2 * ac_true[:N] * P11[:N] / P13[:N] * (1 * (2*np.pi)**2)
ratio = 2 * ac[:N] * P11[:N]/ P13[:N] * (1 * (2*np.pi)**2)
ratio2 = 2 * ac2[:N] * P11[:N] / P13[:N] * (1 * (2*np.pi)**2)
ratio3 = 2 * ac3[:N] * P11[:N] / P13[:N] * (1 * (2*np.pi)**2)
ratio4 = 2 * ac4[:N] * P11[:N] / P13[:N] * (1 * (2*np.pi)**2)
...
line5, = ax.plot(a_list[:N], ratio_true[:N]/fac, c='r', lw=2) #, marker='o') #fit
line1, = ax.plot(a_list[:N], ratio[:N]/fac, c='k', lw=2, zorder=4, ls='dashed') #, marker='o') #fit
line2, = ax.plot(a_list[:N], ratio2[:N]/fac, c='cyan', lw=2, ls='dashed')#, marker='*') #M&W
line3, = ax.plot(a_list[:N], ratio3[:N]/fac, c='magenta', lw=2, ls='dashed')#, marker='v') #B^{+12}

ax.axhline(np.mean(ratio_true[:N]/fac), c='grey', lw=1, ls='dotted')
ax.axhline(np.mean(ratio[:N]/fac), c='grey', lw=1, ls='dotted')

line4, = ax.plot(a_list[:N], ratio4[:N]/fac, c='lightseagreen', lw=2, ls='dashed')#, lw=1.5, marker='') #DDE

handles = [line1, line2, line3, line4, line5, sc_line]#, line6]
labels=[r'from fit to $\langle[\tau]_{\Lambda}\rangle$', r'M\&W', r'$\mathrm{B^{+12}}$', r'DDE', r'from matching $P_{N-}$body', r'$a_{\mathrm{sc}}$']
ax.legend(handles=handles, labels=labels, fontsize=14, bbox_to_anchor=(0.43, 0.425))
...
# plt.savefig('../plots/paper_plots_final/eta_turnover.png', bbox_inches='tight', dpi=300) #ctot2/err/lined/
